...
#if (STBTT_MAX_OVERSAMPLE > 511) || (0 != ((STBTT_MAX_OVERSAMPLE - 1) & STBTT_MAX_OVERSAMPLE))
#error "STBTT_MAX_OVERSAMPLE is invalid"
#endif
...
static uint32_t stbtt__find_table(const uint8_t *data, uint32_t fontstart, const char *tag) {
...
static int32_t stbtt__GetCoverageIndex(const uint8_t *coverageTable, int glyph) {
  ...
  case 1: {
    ...
    int straw,
      needle = glyph;
    while (l <= r) {
      const uint8_t *glyphArray = coverageTable + 4;
      uint16_t glyphID;
      m = (l + r) >> 1;
  ...
  case 2: {
    ...
    const uint8_t *rangeArray = coverageTable + 4;
    // Binary search.
    int32_t l = 0,
            r = rangeCount - 1,
            m;
...
static int32_t stbtt__GetGlyphClass(const uint8_t *classDefTable, int glyph) {
  ...
  case 1: {
    ...
    const uint8_t *classDef1ValueArray = classDefTable + 6;
  ...
  case 2: {
    ...
    const uint8_t *classRangeRecords = classDefTable + 4;
    // Binary search.
    int32_t l = 0,
            r = classRangeCount - 1,
            m;
    int strawStart,
      ...
      needle = glyph;
    while (l <= r) {
      const uint8_t *classRangeRecord;
      m = (l + r) >> 1;
    ...
    for (sti = 0; sti < subTableCount; sti++) {
      ...
      case 1: {
        ...
        if (valueFormat1 == 4 && valueFormat2 == 0) {
          ...
          const uint8_t *pairValueTable = table + pairPosOffset;
          uint16_t pairValueCount = ttUSHORT(pairValueTable);
          const uint8_t *pairValueArray = pairValueTable + 2;
...
static void stbtt__rasterize_sorted_edges(stbtt__bitmap *result, stbtt__edge *e, int n, int vsubsample, ivec2 off, void *userdata) {
  stbtt__hheap hh = { 0, 0, 0};
  stbtt__active_edge *active = NULL;
  int y, j = 0, i;
  float *scanline, *scanline2;
  UNUSED(vsubsample);
  scanline = (float *)malloc((result->size.x * 2 + 1) * sizeof(float));
  scanline2 = scanline + result->size.x;
  y = off.y;
  e[n].y0 = (float)(off.y + result->size.y) + 1;
  while (j < result->size.y) {
    // find center of pixel for this scanline
    float scan_y_top = y + 0.0f;
    float scan_y_bottom = y + 1.0f;
    stbtt__active_edge **step = &active;
    memset(scanline, 0, (result->size.x * 2 + 1) * sizeof(float));
    ...
    while (e->y0 <= scan_y_bottom) {
      if (e->y0 != e->y1) {
        stbtt__active_edge *z = stbtt__new_active(&hh, e, off.x, scan_y_top, userdata);
        if (z != NULL) {
          if (j == 0 && off.y != 0) {
            if (z->ey < scan_y_top) {
    ...
    if (active) stbtt__fill_active_edges_new(scanline, scanline2 + 1, result->size.x, active, scan_y_top);
    {
      float sum = 0;
...
static void stbtt__rasterize(stbtt__bitmap *result, vec2 *pts, int *wcount, int windings, vec2 scale, vec2 shift, ivec2 off, int invert, void *userdata) {
  ...
  for (i = 0; i < windings; ++i) {
    ...
    for (k = 0; k < wcount[i]; j = k++) {
      ...
      e[n].x0 = p[a].x * scale.x +shift.x;
      e[n].y0 = (p[a].y * y_scale_inv +shift.y) * vsubsample;
      e[n].x1 = p[b].x * scale.x +shift.x;
      e[n].y1 = (p[b].y * y_scale_inv +shift.y) * vsubsample;
      ++n;
    }
  }
  ...
  stbtt__rasterize_sorted_edges(result, e, n, vsubsample, off, userdata);
  free(e);
}
static void stbtt__add_point(vec2 *points, int n, float x, float y) {
  if (!points)
...
static int stbtt__tesselate_curve(vec2 *points, int *num_points, float x0, float y0, float x1, float y1, float x2, float y2, float objspace_flatness_squared, int n) {
  // midpoint
...
static void stbtt__tesselate_cubic(vec2 *points, int *num_points, float x0, float y0, float x1, float y1, float x2, float y2, float x3, float y3, float objspace_flatness_squared, int n) {
  ...
  *contour_lengths = (int *)malloc(sizeof(**contour_lengths) * n);
  ...
  for (pass = 0; pass < 2; ++pass) {
    float x = 0,
          y = 0;
    if (pass == 1) {
      points = (vec2 *)malloc(num_points * sizeof(points[0]));
      if (points == NULL)
        goto error;
    }
    num_points = 0;
...
static void stbtt__h_prefilter(stbtt__bitmap gbm, unsigned int kernel_width) {
  unsigned char buffer[STBTT_MAX_OVERSAMPLE] = {0};
  int safe_w = gbm.size.x - kernel_width;
  int j;
  for (j = 0; j < gbm.size.y; ++j) {
    int i;
    unsigned int total;
    ...
    case 2:
      for (i = 0; i <= safe_w; ++i) {
        total += gbm.pixels[i] - buffer[i & STBTT__OVER_MASK];
        buffer[(i + kernel_width) & STBTT__OVER_MASK] = gbm.pixels[i];
        gbm.pixels[i] = (unsigned char)(total / 2);
      }
      break;
    case 3:
      for (i = 0; i <= safe_w; ++i) {
        total += gbm.pixels[i] - buffer[i & STBTT__OVER_MASK];
        buffer[(i + kernel_width) & STBTT__OVER_MASK] = gbm.pixels[i];
        gbm.pixels[i] = (unsigned char)(total / 3);
      }
      break;
    case 4:
      for (i = 0; i <= safe_w; ++i) {
        total += gbm.pixels[i] - buffer[i & STBTT__OVER_MASK];
        buffer[(i + kernel_width) & STBTT__OVER_MASK] = gbm.pixels[i];
        gbm.pixels[i] = (unsigned char)(total / 4);
      }
      break;
    case 5:
      for (i = 0; i <= safe_w; ++i) {
        total += gbm.pixels[i] - buffer[i & STBTT__OVER_MASK];
        buffer[(i + kernel_width) & STBTT__OVER_MASK] = gbm.pixels[i];
        gbm.pixels[i] = (unsigned char)(total / 5);
      }
      break;
    default:
      for (i = 0; i <= safe_w; ++i) {
        total += gbm.pixels[i] - buffer[i & STBTT__OVER_MASK];
        buffer[(i + kernel_width) & STBTT__OVER_MASK] = gbm.pixels[i];
        gbm.pixels[i] = (unsigned char)(total / kernel_width);
      }
      break;
    }
    for (; i < gbm.size.x; ++i) {
      ASSERT(gbm.pixels[i] == 0);
      total -= buffer[i & STBTT__OVER_MASK];
      gbm.pixels[i] = (unsigned char)(total / kernel_width);
    }
    gbm.pixels += gbm.stride;
  }
}
static void stbtt__v_prefilter(stbtt__bitmap gbm, unsigned int kernel_width) {
  unsigned char buffer[STBTT_MAX_OVERSAMPLE] = {0};
  int safe_h = gbm.size.y - kernel_width;
  int j;
  for (j = 0; j < gbm.size.x; ++j) {
    int i;
    unsigned int total;
    memset(buffer, 0, kernel_width);
    total = 0;
    ...
    case 2:
      for (i = 0; i <= safe_h; ++i) {
        total += gbm.pixels[i * gbm.stride] - buffer[i & STBTT__OVER_MASK];
        buffer[(i + kernel_width) & STBTT__OVER_MASK] = gbm.pixels[i * gbm.stride];
        gbm.pixels[i * gbm.stride] = (unsigned char)(total / 2);
      }
      break;
    case 3:
      for (i = 0; i <= safe_h; ++i) {
        total += gbm.pixels[i * gbm.stride] - buffer[i & STBTT__OVER_MASK];
        buffer[(i + kernel_width) & STBTT__OVER_MASK] = gbm.pixels[i * gbm.stride];
        gbm.pixels[i * gbm.stride] = (unsigned char)(total / 3);
      }
      break;
    case 4:
      for (i = 0; i <= safe_h; ++i) {
        total += gbm.pixels[i * gbm.stride] - buffer[i & STBTT__OVER_MASK];
        buffer[(i + kernel_width) & STBTT__OVER_MASK] = gbm.pixels[i * gbm.stride];
        gbm.pixels[i * gbm.stride] = (unsigned char)(total / 4);
      }
      break;
    case 5:
      for (i = 0; i <= safe_h; ++i) {
        total += gbm.pixels[i * gbm.stride] - buffer[i & STBTT__OVER_MASK];
        buffer[(i + kernel_width) & STBTT__OVER_MASK] = gbm.pixels[i * gbm.stride];
        gbm.pixels[i * gbm.stride] = (unsigned char)(total / 5);
      }
      break;
    default:
      for (i = 0; i <= safe_h; ++i) {
        total += gbm.pixels[i * gbm.stride] - buffer[i & STBTT__OVER_MASK];
        buffer[(i + kernel_width) & STBTT__OVER_MASK] = gbm.pixels[i * gbm.stride];
        gbm.pixels[i * gbm.stride] = (unsigned char)(total / kernel_width);
      }
      break;
    }
    for (; i < gbm.size.y; ++i) {
      ASSERT(gbm.pixels[i * gbm.stride] == 0);
      total -= buffer[i & STBTT__OVER_MASK];
      gbm.pixels[i * gbm.stride] = (unsigned char)(total / kernel_width);
    }
    gbm.pixels += 1;
  ...
  for (i = 0; i < nverts; ++i) {
    ...
    if (verts[i].type == STBTT_vcurve) {
      ...
      if (y > ay && y < by && x > ax) {
        ...
        q2[0] = (float)x2;
        q2[1] = (float)y2;
        if ((q0[0] == q1[0] && q0[1] == q1[1]) || (q1[0] == q2[0] && q1[1] == q2[1])) {
          x0 = (int)verts[i - 1].x;
          y0 = (int)verts[i - 1].y;
          x1 = (int)verts[i].x;
          y1 = (int)verts[i].y;
...
static int stbtt__matchpair(uint8_t *fc, uint32_t nm, uint8_t *name, int32_t nlen, int32_t target_id, int32_t next_id) {
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (id == target_id) {
      // find the encoding
      int32_t platform = ttUSHORT(fc + loc + 0),
...
static int stbtt_FindMatchingFont_internal(unsigned char *font_collection, char *name_utf8, int32_t flags) {
  int32_t i;
  for (i = 0;; ++i) {
    int32_t off = stbtt_GetFontOffsetForIndex_internal(font_collection, i);
    if (off < 0)
      return off;
...
// validate ttf data
int stbtt_InitFont(stbtt_fontinfo *info, const unsigned char *data, int fontstart) {
  uint32_t cmap, t;
  int32_t i, numTables;
  info->data = data;
...
int stbtt_FindGlyphIndex(const stbtt_fontinfo *info, int unicode_codepoint) {
  ...
  if (format == 0) {
    ...
    if (unicode_codepoint < bytes - 6)
      return *(uint8_t *)(data + index_map + 6 + unicode_codepoint);
    return 0;
  } else if (format == 6) {
...
const unsigned char *stbtt_FindSVGDoc(const stbtt_fontinfo *info, int gl) {
  int i;
  const uint8_t *data = info->data;
  const uint8_t *svg_doc_list = data + stbtt__get_svg((stbtt_fontinfo *)info);
  int numEntries = ttUSHORT(svg_doc_list);
  const uint8_t *svg_docs = svg_doc_list + 2;
  for (i = 0; i < numEntries; i++) {
    const uint8_t *svg_doc = svg_docs + (12 * i);
...
void stbtt_GetGlyphBitmapBoxSubpixel(const stbtt_fontinfo *font, int glyph, vec2 scale, vec2 shift, ivec2 *ip0, ivec2 *ip1) {
  int x0 = 0, y0 = 0, x1, y1; // =0 suppresses compiler warning
  if (!stbtt_GetGlyphBox(font, glyph, &x0, &y0, &x1, &y1)) {
    // e.g. space character
    if (ip0) *ip0 = IVEC2_ZERO;
    if (ip1) *ip1 = IVEC2_ZERO;
  } else {
...
void stbtt_GetGlyphBitmapBox(const stbtt_fontinfo *font, int glyph, vec2 scale, ivec2 *ip0, ivec2 *ip1) {
  stbtt_GetGlyphBitmapBoxSubpixel(font, glyph, scale, VEC2_ZERO, ip0, ip1);
}
void stbtt_GetCodepointBitmapBoxSubpixel(const stbtt_fontinfo *font, int codepoint, vec2 scale, vec2 shift, ivec2 *ip0, ivec2 *ip1) {
  stbtt_GetGlyphBitmapBoxSubpixel(font, stbtt_FindGlyphIndex(font, codepoint), scale, shift, ip0, ip1);
}
void stbtt_GetCodepointBitmapBox(const stbtt_fontinfo *font, int codepoint, vec2 scale, ivec2 *ip0, ivec2 *ip1) {
  stbtt_GetCodepointBitmapBoxSubpixel(font, codepoint, scale, VEC2_ZERO, ip0, ip1);
}
void stbtt_Rasterize(stbtt__bitmap *result, float flatness_in_pixels, stbtt_vertex *vertices, int num_verts, vec2 scale, vec2 shift, ivec2 off, int invert, void *userdata) {
  int winding_count = 0;
  int *winding_lengths = NULL;
  vec2 *windings = stbtt_FlattenCurves(vertices, num_verts, flatness_in_pixels / MIN(scale.x, scale.y), &winding_lengths, &winding_count, userdata);
  if (windings) {
...
unsigned char *stbtt_GetGlyphBitmapSubpixel(const stbtt_fontinfo *info, vec2 scale, vec2 shift, int glyph, ivec2 *out_size, ivec2 *off) {
  stbtt__bitmap gbm = {0};
  ...
  if (vec2_is0((&scale)) && ((scale.x * scale.y) != 0.0f)) {
    ...
    if (!ivec2_is0((&gbm.size))) {
      ...
      if (gbm.pixels) {
        ...
        stbtt_Rasterize(&gbm, 0.35f, vertices, num_verts, scale, shift, ip0, 1, info->userdata);
      }
    }
  }
  free(vertices);
...
unsigned char *stbtt_GetGlyphBitmap(const stbtt_fontinfo *info, vec2 scale, int glyph, ivec2 *out_size, ivec2 *off) {
  return stbtt_GetGlyphBitmapSubpixel(info, scale, VEC2_ZERO, glyph, out_size, off);
}
void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, vec2 shift, int glyph) {
  ivec2 ip;
...
void stbtt_MakeGlyphBitmap(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, int glyph) {
  stbtt_MakeGlyphBitmapSubpixel(info, gbm, scale, VEC2_ZERO, glyph);
}
unsigned char *stbtt_GetCodepointBitmapSubpixel(const stbtt_fontinfo *info, vec2 scale, vec2 shift, int codepoint, ivec2 *out_size, ivec2 *off) {
  return stbtt_GetGlyphBitmapSubpixel(info, scale, shift, stbtt_FindGlyphIndex(info, codepoint), out_size, off);
}
void stbtt_MakeGlyphBitmapSubpixelPrefilter(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, vec2 shift, int prefilter_x, int prefilter_y, vec2 *sub, int glyph) {
  gbm.size.x -= prefilter_x - 1;
  gbm.size.y -= prefilter_y - 1;
  stbtt_MakeGlyphBitmapSubpixel(info, gbm, scale, shift, glyph);
  if (prefilter_x > 1)
    stbtt__h_prefilter(gbm, prefilter_x);
  if (prefilter_y > 1)
    stbtt__v_prefilter(gbm, prefilter_y);
  sub->x = stbtt__oversample_shift(prefilter_x);
  sub->y = stbtt__oversample_shift(prefilter_y);
}

void stbtt_MakeCodepointBitmapSubpixelPrefilter(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, vec2 shift, int oversample_x, int oversample_y, vec2 *sub, int codepoint) {
  stbtt_MakeGlyphBitmapSubpixelPrefilter(info, gbm, scale, shift, oversample_x, oversample_y, sub, stbtt_FindGlyphIndex(info, codepoint));
}
void stbtt_MakeCodepointBitmapSubpixel(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, vec2 shift, int codepoint) {
  stbtt_MakeGlyphBitmapSubpixel(info, gbm, scale, shift, stbtt_FindGlyphIndex(info, codepoint));
}
unsigned char *stbtt_GetCodepointBitmap(const stbtt_fontinfo *info, vec2 scale, int codepoint, ivec2 *out_size, ivec2 *off) {
  return stbtt_GetCodepointBitmapSubpixel(info, scale, VEC2_ZERO, codepoint, out_size, off);
}
void stbtt_MakeCodepointBitmap(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, int codepoint) {
  stbtt_MakeCodepointBitmapSubpixel(info, gbm, scale, VEC2_ZERO, codepoint);
}
void stbtt_GetBakedQuad(const stbtt_bakedchar *chardata, ivec2 sz, int char_index, vec2 *pos, stbtt_aligned_quad *q, int opengl_fillrule) {
...
void stbtt_GetScaledFontVMetrics(const unsigned char *fontdata, int index, float size, float *ascent, float *descent, float *lineGap) {
  ...
  float scale;
  stbtt_fontinfo info;
  stbtt_InitFont(&info, fontdata, stbtt_GetFontOffsetForIndex_internal(fontdata, index));
...
void stbtt_GetPackedQuad(const stbtt_packedchar *chardata, ivec2 p, int char_index, vec2 *pos, stbtt_aligned_quad *q, int align_to_integer) {
...
unsigned char *stbtt_GetGlyphSDF(const stbtt_fontinfo *info, float scl, int glyph, int padding, unsigned char onedge_value, float pixel_dist_scale, ivec2 *out_size, ivec2 *off) {
  ...
  if (ip0.x == ip1.x || ip0.y == ip1.y) return NULL;
  ip0.x -= padding;
  ip0.y -= padding;
  ip1.x += padding;
  ip1.y += padding;
  size = ivec2_sub(ip1, ip0);
  ...
  scale.y *= -1;
  {
    ...
    for (y = ip0.y; y < ip1.y; ++y) {
      for (x = ip0.x; x < ip1.x; ++x) {
        ...
        val = CLAMP(0, val, 255);
        data[(y - ip0.y) * ip1.x + (x - ip0.x)] = (unsigned char)val;
      }
    }
    free(precompute);
    free(verts);
...
unsigned char *stbtt_GetCodepointSDF(const stbtt_fontinfo *info, float scale, int codepoint, int padding, unsigned char onedge_value, float pixel_dist_scale, ivec2 *out_size, ivec2 *off) {
  return stbtt_GetGlyphSDF(info, scale, stbtt_FindGlyphIndex(info, codepoint), padding, onedge_value, pixel_dist_scale, out_size, off);
}
...
int stbtt_BakeFontBitmap(const unsigned char *data, int offset, float pixel_height, unsigned char *pixels, int pw, int ph, int first_char, int num_chars, stbtt_bakedchar *chardata) {
  ...
  for (i = 0; i < num_chars; ++i) {
    ...
    testv = ivec2_add(st, gbm.size);
    if (testv.y >= (ph - 1)) // check if it fits vertically AFTER potentially moving to next row
      return -i;
    ASSERT(testv.x < pw);
    ASSERT(testv.y < ph);
    gbm.pixels = pixels + st.x + st.y * pw;
    stbtt_MakeGlyphBitmap(&f, gbm, scale, g);
    chardata[i].p0.x = st.x;
    chardata[i].p0.y = st.y;
    chardata[i].p1.x = testv.x;
    chardata[i].p1.y = testv.y;
    chardata[i].xadvance = scale.x * advance;
    chardata[i].off.x = ip.x;
    chardata[i].off.y = ip.y;
    st.x = testv.x + 1;
    if (testv.y + 1 > bottom_y)
      bottom_y = testv.y + 1;
  }
  return bottom_y;
}

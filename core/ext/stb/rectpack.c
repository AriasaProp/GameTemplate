...
static int stbrp__skyline_find_min_y(struct stbrp_node *first, int x0, int width, int *pwaste) {
  ...
  while (node->x < x1) {
    if (node->y > min_y) {
      ...
      visited_width += node->next->x - MAX(node->x, x0);
    } else {
      // add waste area
...
static stbrp__findresult stbrp__skyline_find_best_pos(struct stbrp_context *c, int width, int height) {
  ...
  while (node->x + width <= c->width) {
    ...
    if (global_heuristic == STBRP_HEURISTIC_Skyline_BL_sortHeight) {
...
int stbrp_pack_rects(stbrp_rect *rects, int num_rects, int width, int height) {

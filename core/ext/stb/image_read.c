...
static void stbi__start_mem(stbi__context *s, ubyte const *buffer, int len) {
  s->io.read = NULL;
  s->read_from_callbacks = 0;
  s->callback_already_read = 0;
  s->img_buffer = s->img_buffer_original = (ubyte *)buffer;
  s->img_buffer_end = s->img_buffer_original_end = (ubyte *)buffer + len;
}
...
static void *stbi__read_gif_main(stbi__context *s, int **delays, int *x, int *y, int *z, int *comp, int req_comp);
...
static int stbi__mul2shrts_valid(int a, int b) {
  if (b == 0 || b == -1)
...
#define stbi__err(...) (stb_set_error(__VA_ARGS__), 0)

#ifndef STBI_NO_LINEAR
static float *stbi__ldr_to_hdr(ubyte *data, int x, int y, int comp);
#endif

#ifndef STBI_NO_HDR
static ubyte *stbi__hdr_to_ldr(float *data, int x, int y, int comp);
#endif

static void *stbi__read_main(stbi__context *s, int *x, int *y, int *comp, int req_comp, stbi__result_info *ri, int bpc) {
...
// try them later
  ...
  return (ubyte *)(size_t)stbi__err("unknown image type : Image not of any known type, or corrupt");
}

static ubyte *stbi__convert_16_to_8(ushrt *orig, int w, int h, int channels) {
  int i;
  int img_len = w * h * channels;
  ubyte *reduced;

  reduced = (ubyte *)malloc(img_len);
  if (reduced == NULL)
...
static ushrt *stbi__convert_8_to_16(ubyte *orig, int w, int h, int channels) {
  int i;
  int img_len = w * h * channels;
  ushrt *enlarged;

  enlarged = (ushrt *)malloc(img_len * 2);
  if (enlarged == NULL)
...
static void stbi__vertical_flip(void *image, int w, int h, int bytes_per_pixel) {
  ...
  for (row = 0; row < (h >> 1); row++) {
    ubyte *row0 = bytes + row * bytes_per_row;
    ubyte *row1 = bytes + (h - row - 1) * bytes_per_row;
    // swap row0 with row1
...
#endif
#endif // VERTICALY_FLIP

static ubyte *stbi__read_and_postprocess_8bit(stbi__context *s, int *x, int *y, int *comp, int req_comp) {
  stbi__result_info ri;
  void *result = stbi__read_main(s, x, y, comp, req_comp, &ri, 8);
  ...
  if (ri.bits_per_channel != 8) {
    result = stbi__convert_16_to_8((ushrt *)result, *x, *y, req_comp == 0 ? *comp : req_comp);
  ...
  stbi__vertical_flip(result, *x, *y, channels * sizeof(ubyte));
#endif // VERTICALLY_FLIP

  return (ubyte *)result;
}

static ushrt *stbi__read_and_postprocess_16bit(stbi__context *s, int *x, int *y, int *comp, int req_comp) {
  stbi__result_info ri;
  void *result = stbi__read_main(s, x, y, comp, req_comp, &ri, 16);
  ...
  if (ri.bits_per_channel != 16) {
    result = stbi__convert_8_to_16((ubyte *)result, *x, *y, req_comp == 0 ? *comp : req_comp);
...
#ifdef VERTICALLY_FLIP
  int channels = req_comp ? req_comp : *comp;
  stbi__vertical_flip(result, *x, *y, channels * sizeof(ushrt));
#endif // VERTICALLY_FLIP
...
ushrt *stbi_read_16_from_memory(ubyte const *buffer, int len, int *x, int *y, int *channels_in_file, int desired_channels) {
  ...
  return stbi__read_and_postprocess_16bit(&s, x, y, channels_in_file, desired_channels);
}
...
ubyte *stbi_read_memory(ubyte const *buffer, int len, int *x, int *y, int *comp, int req_comp) {
  stbi__context s;
  stbi__start_mem(&s, buffer, len);
...
static void stbi__asset_skip(void *user, int n) {
  assetSeek(CAST(asset)user, n);
}
static bool stbi__asset_eof (void *user) {
  return assetLength(CAST(asset)user) == 0;
}
ubyte *stbi_read_asset(const char *file, int *x, int *y, int *comp, int req_comp) {
  stbi__context s;
  stbi_io_callbacks cf = {
    .read = stbi__asset_read,
    .skip = stbi__asset_skip,
    .eof  = stbi__asset_eof,
  };
  asset A = openAsset(file);
  stbi__start_callbacks(&s, &cf, CAST(void*)A);
  ubyte *res = stbi__read_and_postprocess_8bit(&s, x, y, comp, req_comp);
  assetClose(A);
  return res;
}

ubyte *stbi_read_callbacks(stbi_io_callbacks const *clbk, void *user, int *x, int *y, int *comp, int req_comp) {
  stbi__context s;
  stbi__start_callbacks(&s, (stbi_io_callbacks *)clbk, user);
  return stbi__read_and_postprocess_8bit(&s, x, y, comp, req_comp);
}
...
ubyte *stbi_read_gif_from_memory(ubyte const *buffer, int len, int **delays, int *x, int *y, int *z, int *comp, int req_comp) {
  ...
  result = (ubyte *)stbi__read_gif_main(&s, delays, x, y, z, comp, req_comp);
#ifdef VERTICALLY_FLIP
...
#ifndef STBI_NO_LINEAR
static float *stbi__readf_main(stbi__context *s, int *x, int *y, int *comp, int req_comp) {
  ubyte *data;
#ifndef STBI_NO_HDR
  if (stbi__hdr_test(s)) {
    stbi__result_info ri;
...
#endif
  data = stbi__read_and_postprocess_8bit(s, x, y, comp, req_comp);
  if (data)
    return stbi__ldr_to_hdr(data, *x, *y, req_comp ? req_comp : *comp);
  return (float *)(size_t)stbi__err("unknown image type : Image not of any known type, or corrupt");
}

float *stbi_readf_from_memory(ubyte const *buffer, int len, int *x, int *y, int *comp, int req_comp) {
  stbi__context s;
  stbi__start_mem(&s, buffer, len);
  return stbi__readf_main(&s, x, y, comp, req_comp);
}

float *stbi_readf_from_callbacks(stbi_io_callbacks const *clbk, void *user, int *x, int *y, int *comp, int req_comp) {
  stbi__context s;
  stbi__start_callbacks(&s, (stbi_io_callbacks *)clbk, user);
  return stbi__readf_main(&s, x, y, comp, req_comp);
}
...
float *stbi_readf(char const *filename, int *x, int *y, int *comp, int req_comp) {
  ...
  stbi__context s;
  stbi__start_callbacks(&s, &stbi__stdio_callbacks, (void *)f);
  result = stbi__readf_main(&s, x, y, comp, req_comp);
  fclose(f);
  return result;
}
...
inline static ubyte stbi__get8(stbi__context *s) {
...
// nothing
#else
static int stbi__getn(stbi__context *s, ubyte *buffer, int n) {
  if (s->io.read) {
    int blen = (int)(s->img_buffer_end - s->img_buffer);
    if (blen < n) {
      int res, count;
...
#define STBI__BYTECAST(x) ((ubyte)((x) & 255)) // truncate int to byte without warnings
...
static ubyte stbi__compute_y(int r, int g, int b) {
  return (ubyte)(((r * 77) + (g * 150) + (29 * b)) >> 8);
}
#endif
...
// nothing
#else
static ubyte *stbi__convert_format(ubyte *data, int img_n, int req_comp, uint x, uint y) {
  int i, j;
  ubyte *good;
...
// nothing
#else
static ushrt stbi__compute_y_16(int r, int g, int b) {
...
// nothing
#else
static ushrt *stbi__convert_format16(ushrt *data, int img_n, int req_comp, uint x, uint y) {
  int i, j;
  ushrt *good;
...
// one go.
static void stbi__build_fast_ac(shrt *fast_ac, stbi__huffman *h) {
  int i;
...
static void stbi__grow_buffer_unsafe(stbi__jpeg *j) {
  do {
    uint b = j->nomore ? 0 : stbi__get8(j->s);
    if (b == 0xff) {
...
// (1 << n) - 1
static const uint32 stbi__bmask[17] = {0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535};
...
static const ubyte stbi__jpeg_dezigzag[64 + 15] =
  {
    0, 1, 8, 16, 9, 2, 3, 10,
...
static int stbi__jpeg_decode_block(stbi__jpeg *j, shrt data[64], stbi__huffman *hdc, stbi__huffman *hac, shrt *fac, int b, ushrt *dequant) {
  int diff, dc, k;
  int t;
  ...
  if (!stbi__mul2shrts_valid(dc, dequant[0]))
  ...
  do {
    ...
    if (r) {              // fast-AC path
      ...
      if (s == 0) {
        ...
        // decode into unzigzag'd location
        zig = stbi__jpeg_dezigzag[k++];
        data[zig] = (shrt)(stbi__extend_receive(j, s) * dequant[zig]);
      }
    }
  } while (k < 64);
  return 1;
...
static int stbi__jpeg_decode_block_prog_dc(stbi__jpeg *j, shrt data[64], stbi__huffman *hdc, int b) {
  int diff, dc;
  int t;
  if (j->spec_end != 0)
    return stbi__err("can't merge dc and ac : Corrupt JPEG");
  ...
  if (j->succ_high == 0) {
    ...
    dc = j->img_comp[b].dc_pred + diff;
    j->img_comp[b].dc_pred = dc;
    if (!stbi__mul2shrts_valid(dc, 1 << j->succ_low))
      return stbi__err("can't merge dc and ac : Corrupt JPEG");
...
static int stbi__jpeg_decode_block_prog_ac(stbi__jpeg *j, shrt data[64], stbi__huffman *hac, shrt *fac) {
  int k;
  if (j->spec_start == 0)
    return stbi__err("can't merge dc and ac : Corrupt JPEG");
  ...
  if (j->succ_high == 0) {
    ...
    do {
      ...
      if (r) {              // fast-AC path
        ...
        if (s == 0) {
          ...
          k += r;
          zig = stbi__jpeg_dezigzag[k++];
          data[zig] = (shrt)(stbi__extend_receive(j, s) * (1 << shift));
        }
      }
    } while (k <= j->spec_end);
  } else {
    ...
    if (j->eob_run) {
      ...
      for (k = j->spec_start; k <= j->spec_end; ++k) {
        shrt *p = &data[stbi__jpeg_dezigzag[k]];
        if (*p != 0)
          if (stbi__jpeg_get_bit(j))
            if ((*p & bit) == 0) {
              if (*p > 0)
...
inline static ubyte stbi__clamp(int x) {
...
static ubyte stbi__get_marker(stbi__jpeg *j) {
  ubyte x;
...
static void stbi__jpeg_dequantize(shrt *data, ushrt *dequant) {
  int i;
  for (i = 0; i < 64; ++i)
    data[i] *= dequant[i];
}
...
static int stbi__process_marker(stbi__jpeg *z, int m) {
  ...
  case 0xDB: // DQT - define quantization table
    ...
    while (L > 0) {
      ...
      for (i = 0; i < 64; ++i)
        z->dequant[t][stbi__jpeg_dezigzag[i]] = (ushrt)(sixteen ? stbi__get16be(z->s) : stbi__get8(z->s));
      L -= (sixteen ? 129 : 65);
    }
    return L == 0;
...
static int stbi__process_frame_header(stbi__jpeg *z, int scan) {
  ...
  if (s->img_y > MAX_DIMENSIONS)
    return stbi__err("Very large image (corrupt?)");
  ...
  if (!stbi__mad3sizes_valid(s->img_x, s->img_y, s->img_n, 0))
    return stbi__err("Image too large to decode");
  ...
  for (i = 0; i < s->img_n; ++i) {
    ...
    z->img_comp[i].data = (ubyte *)(((size_t)z->img_comp[i].raw_data + 15) & ~15);
    if (z->progressive) {
      // w2, h2 are multiples of 8 (see above)
      z->img_comp[i].coeff_w = z->img_comp[i].w2 / 8;
      z->img_comp[i].coeff_h = z->img_comp[i].h2 / 8;
      z->img_comp[i].raw_coeff = stbi__malloc_mad3(z->img_comp[i].w2, z->img_comp[i].h2, sizeof(shrt), 15);
      if (z->img_comp[i].raw_coeff == NULL)
        return stbi__free_jpeg_components(z, i + 1, stbi__err("outofmem : Out of memory"));
      z->img_comp[i].coeff = (shrt *)(((size_t)z->img_comp[i].raw_coeff + 15) & ~15);
    }
  }
...
static ubyte stbi__skip_jpeg_junk_at_end(stbi__jpeg *j) {
  // some JPEGs have junk at end, skip over it but if we find what looks
  // like a valid marker, resume there
  while (!stbi__at_eof(j->s)) {
    ubyte x = stbi__get8(j->s);
...
static ubyte *stbi__resample_row_v_2(ubyte *out, ubyte *in_near, ubyte *in_far, int w, int hs) {
...
static ubyte *stbi__resample_row_h_2(ubyte *out, ubyte *in_near, ubyte *in_far, int w, int hs) {
...
static ubyte stbi__blinn_8x8(ubyte x, ubyte y) {
  uint t = x * y + 128;
  return (ubyte)((t + (t >> 8)) >> 8);
}

static ubyte *load_jpeg_image(stbi__jpeg *z, int *out_x, int *out_y, int *comp, int req_comp) {
  int n, decode_n, is_rgb;
  z->s->img_n = 0; // make stbi__cleanup_jpeg safe

  // validate req_comp
  if (req_comp < 0 || req_comp > 4)
    return (ubyte *)(size_t)stbi__err("bad req_comp : Internal error");
  ...
  // resample and color-convert
  {
    ...
    for (k = 0; k < decode_n; ++k) {
      ...
      z->img_comp[k].linebuf = (ubyte *)malloc(z->s->img_x + 3);
    ...
    output = (ubyte *)stbi__malloc_mad3(n, z->s->img_x, z->s->img_y, 1);
    if (!output) {
      stbi__cleanup_jpeg(z);
      return (ubyte *)(size_t)stbi__err("outofmem : Out of memory");
    }
    ...
    for (j = 0; j < z->s->img_y; ++j) {
      ...
      if (n >= 3) {
        ...
        if (is_rgb) {
          ...
          for (i = 0; i < z->s->img_x; ++i) {
            ubyte m = coutput[3][i];
            ubyte r = stbi__blinn_8x8(coutput[0][i], m);
            ubyte g = stbi__blinn_8x8(coutput[1][i], m);
            ubyte b = stbi__blinn_8x8(coutput[2][i], m);
...
static int stbi__check_png_header(stbi__context *s) {
  ubyte buf[8];
  if (!stbi__getn(s, buf, 8)) {
    return stbi__err("PNG Header not long enough");
  } else if (memcmp(PNG_SIGNATURE, buf, 8)) {
    return stbi__err("PNG Signature %x != %x", *(uint64_t *)PNG_SIGNATURE, *(uint64_t *)buf);
  }
  return 1;
}
...
typedef struct {
  stbi__context *s;
  ubyte *idata, *expanded, *out, depth;
} stbi__png;
...
static const ubyte stbi__depth_scale_table[9] = {0, 0xff, 0x55, 0, 0x11, 0, 0, 0, 0x01};

// adds an extra all-255 alpha channel
// dest == src is legal
// img_n must be 1 or 3
static void stbi__create_png_alpha_expand8(ubyte *dest, ubyte *src, uint32 x, int img_n) {
  int i;
...
static int stbi__create_png_image_raw(stbi__png *a, ubyte *raw, uint32 raw_len, int out_n, uint32 x, uint32 y, int depth, int color) {
  ...
  a->out = (ubyte *)stbi__malloc_mad3(x, y, output_bytes, 0); // extra bytes to write off the end into
  if (!a->out)
    return stbi__err("outofmem : Out of memory");
  ...
  filter_buf = (ubyte *)stbi__malloc_mad2(img_width_bytes, 2, 0);
  if (!filter_buf)
    return stbi__err("outofmem : Out of memory");
  ...
  for (j = 0; j < y; ++j) {
    ...
    if (depth < 8) {
      ubyte scale = (color == 0) ? stbi__depth_scale_table[depth] : 1; // scale grayscale values to 0..255 range
      ubyte *in = cur;
      ubyte *out = dest;
      ubyte inb = 0;
      uint32 nsmp = x * img_n;
...
static int stbi__create_png_image(stbi__png *a, ubyte *image_data, uint32 image_data_len, int out_n, int depth, int color, int interlaced) {
  int bytes = (depth == 16 ? 2 : 1);
  ...
  // de-interlacing
  final = (ubyte *)stbi__malloc_mad3(a->s->img_x, a->s->img_y, out_bytes, 0);
  if (!final)
    return stbi__err("outofmem : Out of memory");
  for (p = 0; p < 7; ++p) {
...
static int stbi__compute_transparency(stbi__png *z, ubyte tc[3], int out_n) {
  stbi__context *s = z->s;
...
static int stbi__compute_transparency16(stbi__png *z, ushrt tc[3], int out_n) {
  stbi__context *s = z->s;
  uint32 i, pixel_count = s->img_x * s->img_y;
  ushrt *p = (ushrt *)z->out;
...
static int stbi__expand_png_palette(stbi__png *a, ubyte *palette, int UNUSED_ARG(len), int pal_img_n) {
  uint32 i, pixel_count = a->s->img_x * a->s->img_y;
  ubyte *p = (ubyte *)stbi__malloc_mad2(pixel_count, pal_img_n, 0), *temp_out, *orig = a->out;
  if (!p)
...
static void stbi__de_iphone(stbi__png *z) {
  ...
  if (s->img_out_n == 3) { // convert bgr to rgb
    ...
    for (i = 0; i < pixel_count; ++i) {
      ...
      if (p[3] > 1) {
        ubyte half = p[3] >> 1;
        p[0] = (p[2] * 255 + half) / p[3];
        p[1] = (p[1] * 255 + half) / p[3];
        p[2] = (t * 255 + half) / p[3];
      } else {
        p[0] = p[2];
        p[2] = t;
...
static int stbi__parse_png_file(stbi__png *z, int scan, int req_comp) {
  ubyte palette[1024], pal_img_n = 0;
  ubyte has_trans = 0, tc[3] = {0};
  ushrt tc16[3];
  uint32 ioff = 0, idata_limit = 0, i, j, pal_len = 0;
  int k, interlace = 0, color = 0, is_iphone = 0;
  stbi__context *s = z->s;
  ...
  uint32 pngchunk_len = stbi__get32be(s);
  uint32 pngchunk_type = stbi__get32be(s);
  if (pngchunk_len != 13)
    return stbi__err("bad IHDR/first chunk len");
  if (pngchunk_type != STBI__PNG_TYPE('I', 'H', 'D', 'R'))
    return stbi__err("first png chunk is not IHDR");
  s->img_x = stbi__get32be(s);
  s->img_y = stbi__get32be(s);
  if (!s->img_x || !s->img_y || (s->img_x > MAX_DIMENSIONS) || (s->img_y > MAX_DIMENSIONS))
    return stbi__err("Image size is invalid");
  z->depth = stbi__get8(s);
  if (!z->depth || (z->depth > 16) || (z->depth & (z->depth - 1)))
    return stbi__err("PNG %d bit-depth is unsupported", z->depth);
  color = stbi__get8(s);
  switch (color) {
  case 0:
    break;
  ...
  if (stbi__get8(s))
    return stbi__err("unsupport compression method");
  if (stbi__get8(s))
    return stbi__err("unsupport filter method");
  interlace = stbi__get8(s);
  if (interlace > 1)
    return stbi__err("unsupport interlace method");
  if (!pal_img_n) {
  ...
  stbi__get32be(s); // skip crc
                    // even with SCAN_header, have to scan to see if we have a tRNS

  for (;;) {
    pngchunk_len = stbi__get32be(s);
    pngchunk_type = stbi__get32be(s);
    switch (pngchunk_type) {
    case STBI__PNG_TYPE('I', 'H', 'D', 'R'):
      return stbi__err("PNG has multiple IHDR chunk");
    case STBI__PNG_TYPE('C', 'g', 'B', 'I'):
      is_iphone = 1;
      stbi__skip(s, pngchunk_len);
      break;
    case STBI__PNG_TYPE('P', 'L', 'T', 'E'): {
      if ((pngchunk_len > 256 * 3) || (pngchunk_len % 3))
        return stbi__err("PNG invalid PLTE chunk");
      pal_len = pngchunk_len / 3;
      for (i = 0, j = pal_len * 4; i < pal_len; i += 4) {
        palette[i + 0] = stbi__get8(s);
        palette[i + 1] = stbi__get8(s);
        palette[i + 2] = stbi__get8(s);
        palette[i + 3] = 255;
      }
      break;
    }
    case STBI__PNG_TYPE('t', 'R', 'N', 'S'): {
      ...
      if (pal_img_n) {
        ...
        if (pngchunk_len > pal_len)
          return stbi__err("bad tRNS len");
        pal_img_n = 4;
        for (i = 0, j = pal_len * 4; i < pal_len; i += 4)
          palette[i + 3] = stbi__get8(s);
      } else {
        if (!(s->img_n & 1))
          return stbi__err("tRNS with alpha");
        if (pngchunk_len != (uint32)s->img_n * 2)
          return stbi__err("bad tRNS len");
        has_trans = 1;
        ...
        if (z->depth == 16) {
          for (k = 0; k < s->img_n && k < 3; ++k) // extra loop test to suppress false GCC warning
            tc16[k] = (ushrt)stbi__get16be(s); // copy the values as-is
        } else {
    ...
    case STBI__PNG_TYPE('I', 'D', 'A', 'T'): {
      ...
      if (pngchunk_len > (1u << 30))
        return stbi__err("IDAT size limit : IDAT section larger than 2^30 bytes");
      if ((int)(ioff + pngchunk_len) < (int)ioff)
        return 0;
      if (ioff + pngchunk_len > idata_limit) {
        if (idata_limit == 0)
      ...
      if (!stbi__getn(s, z->idata + ioff, pngchunk_len))
    ...
    case STBI__PNG_TYPE('I', 'E', 'N', 'D'): {
      ...
      uint32 bpl = (s->img_x * z->depth + 7) / 8; // bytes per line, per component
      uint32 raw_len = bpl * s->img_y * s->img_n /* pixels */ + s->img_y /* filter mode per row */;
...
#ifdef IPHONE_PNG_TO_RGB
      if (is_iphone && s->img_out_n > 2)
        stbi__de_iphone(z);
#endif // IPHONE_PNG_TO_RGB
    ...
    default:
      // if critical, fail
      if (!(pngchunk_type & (1 << 29))) {
        return stbi__err("PNG: %08x chunk type unknown!", pngchunk_type);
      }
      stbi__skip(s, pngchunk_len);
      break;
    }
...
static void *stbi__do_png(stbi__png *p, int *x, int *y, int *n, int req_comp, stbi__result_info *ri) {
  ...
  if (stbi__parse_png_file(p, STBI__SCAN_load, req_comp)) {
    if (p->depth <= 8)
      ...
      return (ubyte *)(size_t)stbi__err("bad bits_per_channel : PNG not supported: unsupported color depth");
    result = p->out;
    p->out = NULL;
    if (req_comp && req_comp != p->s->img_out_n) {
      if (ri->bits_per_channel == 8)
        result = stbi__convert_format((ubyte *)result, p->s->img_out_n, req_comp, p->s->img_x, p->s->img_y);
      else
        result = stbi__convert_format16((ushrt *)result, p->s->img_out_n, req_comp, p->s->img_x, p->s->img_y);
...
static void *stbi__bmp_parse_header(stbi__context *s, stbi__bmp_data *info) {
  ...
  if (stbi__get8(s) != 'B' || stbi__get8(s) != 'M')
    return (ubyte *)(size_t)stbi__err("not BMP : Corrupt BMP");
  stbi__get32le(s); // discard filesize
  stbi__get16le(s); // discard reserved
  stbi__get16le(s); // discard reserved
  ...
  if (info->offset < 0)
    return (ubyte *)(size_t)stbi__err("bad BMP : bad BMP");

  if (hsz != 12 && hsz != 40 && hsz != 56 && hsz != 108 && hsz != 124)
    return (ubyte *)(size_t)stbi__err("unknown BMP : BMP type not supported: unknown");
  if (hsz == 12) {
  ...
  if (hsz != 12) {
    ...
    if (compress == 1 || compress == 2)
      return (ubyte *)(size_t)stbi__err("BMP RLE : BMP type not supported: RLE");
    if (compress >= 4)
      return (ubyte *)(size_t)stbi__err("BMP JPEG/PNG : BMP type not supported: unsupported compression"); // this includes PNG/JPEG modes
    if (compress == 3 && info->bpp != 16 && info->bpp != 32)
      return (ubyte *)(size_t)stbi__err("bad BMP : bad BMP"); // bitfields requires 16 or 32 bits/pixel
...
static void *stbi__bmp_load(stbi__context *s, int *x, int *y, int *comp, int req_comp, stbi__result_info *ri) {
  ...
  if (s->img_y > MAX_DIMENSIONS)
    return (ubyte *)(size_t)stbi__err("Very large image (corrupt?)");
  if (s->img_x > MAX_DIMENSIONS)
    return (ubyte *)(size_t)stbi__err("Very large image (corrupt?)");
  ...
  if (psize == 0) {
    ...
    if (bytes_read_so_far <= 0 || bytes_read_so_far > header_limit) {
      return (ubyte *)(size_t)stbi__err("bad header : Corrupt BMP");
    }
    ...
    if (info.offset < bytes_read_so_far || info.offset - bytes_read_so_far > extra_data_limit) {
      return (ubyte *)(size_t)stbi__err("bad offset : Corrupt BMP");
    } else {
  ...
  if (!stbi__mad3sizes_valid(target, s->img_x, s->img_y, 0))
    return (ubyte *)(size_t)stbi__err("Corrupt BMP");

  out = (ubyte *)stbi__malloc_mad3(target, s->img_x, s->img_y, 0);
  if (!out)
    return (ubyte *)(size_t)stbi__err("outofmem : Out of memory");
  if (info.bpp < 16) {
    int z = 0;
...
// by Jonathan Dummer
#ifndef STBI_NO_TGA
// returns 3 or whatever, 0 on error
...
static void stbi__tga_read_rgb16(stbi__context *s, ubyte *out) {
  ushrt px = (ushrt)stbi__get16le(s);
...
static void *stbi__tga_load(stbi__context *s, int *x, int *y, int *comp, int req_comp, stbi__result_info *ri) {
  ...
  UNUSED(ri);
  UNUSED(tga_x_origin); // @TODO
  UNUSED(tga_y_origin); // @TODO

  if (tga_height > MAX_DIMENSIONS)
    return (ubyte *)(size_t)stbi__err("Very large image (corrupt?)");
  if (tga_width > MAX_DIMENSIONS)
    return (ubyte *)(size_t)stbi__err("Very large image (corrupt?)");
  ...
  if (!tga_comp) // shouldn't really happen, stbi__tga_test() should have ensured basic consistency
    return (ubyte *)(size_t)stbi__err("bad format : Can't find out TGA pixelformat");
  ...
  *y = tga_height;
  ...
  if (!stbi__mad3sizes_valid(tga_width, tga_height, tga_comp, 0))
    return (ubyte *)(size_t)stbi__err("Corrupt TGA");

  tga_data = (ubyte *)stbi__malloc_mad3(tga_width, tga_height, tga_comp, 0);
  if (!tga_data)
    return (ubyte *)(size_t)stbi__err("outofmem : Out of memory");
  ...
  if (!tga_indexed && !tga_is_RLE && !tga_rgb16) {
    for (i = 0; i < tga_height; ++i) {
      ...
      ubyte *tga_row = tga_data + row * tga_width * tga_comp;
...
static int stbi__psd_decode_rle(stbi__context *s, ubyte *p, int pixelCount) {
  int count, nleft, len;
...
static void *stbi__psd_load(stbi__context *s, int *x, int *y, int *comp, int req_comp, stbi__result_info *ri, int bpc) {
  ...
  if (stbi__get32be(s) != 0x38425053) // "8BPS"
    return (ubyte *)(size_t)stbi__err("not PSD : Corrupt PSD image");

  // Check file type version.
  if (stbi__get16be(s) != 1)
    return (ubyte *)(size_t)stbi__err("wrong version : Unsupported version of PSD image");
  ...
  if (channelCount < 0 || channelCount > 16)
    return (ubyte *)(size_t)stbi__err("wrong channel count : Unsupported number of channels in PSD image");
  ...
  if (bitdepth != 8 && bitdepth != 16)
    return (ubyte *)(size_t)stbi__err("unsupported bit depth : PSD bit depth is not 8 or 16 bit");
  ...
  if (stbi__get16be(s) != 3)
    return (ubyte *)(size_t)stbi__err("wrong color format : PSD is not in RGB color format");
  ...
  if (compression > 1)
    return (ubyte *)(size_t)stbi__err("bad compression : PSD has an unknown compression format");

  // Check size
  if (!stbi__mad3sizes_valid(4, w, h, 0))
    return (ubyte *)(size_t)stbi__err("Corrupt PSD");

  // Create the destination image.

  if (!compression && bitdepth == 16 && bpc == 16) {
    out = (ubyte *)stbi__malloc_mad3(8, w, h, 0);
    ri->bits_per_channel = 16;
  } else
    out = (ubyte *)malloc(4 * w * h);
    ...
    for (channel = 0; channel < 4; channel++) {
      if (channel >= channelCount) {
        ...
        if (ri->bits_per_channel == 16) { // output bpc
          ushrt *q = ((ushrt *)out) + channel;
          for (i = 0; i < pixelCount; i++, q += 4)
            *q = (ushrt)stbi__get16be(s);
        } else {
          ubyte *p = out + channel;
          if (bitdepth == 16) { // input bpc
            for (i = 0; i < pixelCount; i++, p += 4)
              *p = (ubyte)(stbi__get16be(s) >> 8);
          } else {
            for (i = 0; i < pixelCount; i++, p += 4)
              *p = stbi__get8(s);
          }
  ...
  if (req_comp && req_comp != 4) {
    if (ri->bits_per_channel == 16)
      out = (ubyte *)stbi__convert_format16((ushrt *)out, 4, req_comp, w, h);
    else
...
static ubyte *stbi__readval(stbi__context *s, int channel, ubyte *dest) {
  int mask = 0x80, i;

  for (i = 0; i < 4; ++i, mask >>= 1) {
    if (channel & mask) {
      if (stbi__at_eof(s))
        return (ubyte *)(size_t)stbi__err("bad file : PIC file too shrt");
      dest[i] = stbi__get8(s);
    }
  }
...
static void stbi__copyval(int channel, ubyte *dest, const ubyte *src) {
  int mask = 0x80, i;
...
static ubyte *stbi__pic_load_core(stbi__context *s, int width, int height, int *comp, ubyte *result) {
  ...
  do {
    ...
    if (num_packets == sizeof(packets) / sizeof(packets[0]))
      return (ubyte *)(size_t)stbi__err("bad format : too many packets");
    ...
    if (stbi__at_eof(s))
      return (ubyte *)(size_t)stbi__err("bad file : file too shrt (reading packets)");
    if (packet->size != 8)
      return (ubyte *)(size_t)stbi__err("bad format : packet isn't 8bpp");
  } while (chained);
  ...
  *comp = (act_comp & 0x10 ? 4 : 3); // has alpha channel?
  ...
  for (y = 0; y < height; ++y) {
    ...
    for (packet_idx = 0; packet_idx < num_packets; ++packet_idx) {
      ...
      switch (packet->type) {
      default:
        return (ubyte *)(size_t)stbi__err("bad format : packet has bad compression type");
      ...
      case 1: // Pure RLE
      {
        ...
        while (left > 0) {
          ...
          count = stbi__get8(s);
          if (stbi__at_eof(s))
            return (ubyte *)(size_t)stbi__err("bad file : file too shrt (pure read count)");
      ...
      case 2: { // Mixed RLE
        ...
        while (left > 0) {
          ...
          if (stbi__at_eof(s))
            return (ubyte *)(size_t)stbi__err("bad file : file too shrt (mixed read count)");
          ...
          if (count >= 128) { // Repeated
            ...
            if (count == 128)
              ...
              count -= 127;
            if (count > left)
              return (ubyte *)(size_t)stbi__err("bad file : scanline overrun");
...
static void *stbi__pic_load(stbi__context *s, int *px, int *py, int *comp, int req_comp, stbi__result_info *ri) {
  ...
  if (stbi__at_eof(s))
    return (ubyte *)(size_t)stbi__err("bad file : file too shrt (pic header)");
  if (!stbi__mad3sizes_valid(x, y, 4, 0))
    return (ubyte *)(size_t)stbi__err("PIC image too large to decode");
  ...
  result = (ubyte *)stbi__malloc_mad3(x, y, 4, 0);
  if (!result)
    return (ubyte *)(size_t)stbi__err("outofmem : Out of memory");
...
static void stbi__gif_parse_colortable(stbi__context *s, ubyte pal[256][4], int num_entries, int transp) {
  int i;
...
static void stbi__out_gif_code(stbi__gif *g, ushrt code) {
  ubyte *p, *c;
  int idx;
...
static ubyte *stbi__process_gif_raster(stbi__context *s, stbi__gif *g) {
  ubyte lzw_cs;
  int32 len, init_code;
  uint32 first;
  ...
  for (;;) {
    if (valid_bits < codesize) {
      ...
      if (code == clear) { // clear code
        ...
        if (first) {
          return (ubyte *)(size_t)stbi__err("no clear code : Corrupt GIF");
        }

        if (oldcode >= 0) {
          p = &g->codes[avail++];
          if (avail > 8192) {
            return (ubyte *)(size_t)stbi__err("too many codes : Corrupt GIF");
          }
          ...
          return (ubyte *)(size_t)stbi__err("illegal code in raster : Corrupt GIF");

        stbi__out_gif_code(g, (ushrt)code);
...
static ubyte *stbi__gif_load_next(stbi__context *s, stbi__gif *g, int *comp, int req_comp, ubyte *two_back) {
  int dispose;
  int first_frame;
  int pi;
  int pcount;
  ...
  if (g->out == 0) {
    ...
    if (!stbi__mad3sizes_valid(4, g->w, g->h, 0))
      return (ubyte *)(size_t)stbi__err("GIF image is too large");
    pcount = g->w * g->h;
    g->out = (ubyte *)malloc(4 * pcount);
    g->background = (ubyte *)malloc(4 * pcount);
    g->history = (ubyte *)malloc(pcount);
  ...
  for (;;) {
    ...
    case 0x2C: /* Image Descriptor */
    {
      ...
      h = stbi__get16le(s);
      if (((x + w) > (g->w)) || ((y + h) > (g->h)))
        return (ubyte *)(size_t)stbi__err("bad Image Descriptor : Corrupt GIF");
      ...
      if (g->lflags & 0x80) {
        ...
        return (ubyte *)(size_t)stbi__err("missing color table : Corrupt GIF");
    ...
    default:
      return (ubyte *)(size_t)stbi__err("unknown code : Corrupt GIF");
    }
  }
}
...
static void *stbi__read_gif_main(stbi__context *s, int **delays, int *x, int *y, int *z, int *comp, int req_comp) {
  if (stbi__gif_test(s)) {
      ...
      if (u) {
        ...
        *y = g.h;
        ...
        if (out) {
          ...
          out = (ubyte *)malloc(layers * stride);
          if (!out)
            return stbi__read_gif_main_outofmem(&g, out, delays);
          out_size = layers * stride;
          if (delays) {
    ...
    *z = layers;
    ...
    return (ubyte *)(size_t)stbi__err("not GIF : Image was not as a gif type.");
  }
}
...
static void stbi__hdr_convert(float *output, ubyte *input, int req_comp) {
  if (input[3] != 0) {
    float f1;
    // Exponent
...
static float *stbi__hdr_load(stbi__context *s, int *x, int *y, int *comp, int req_comp, stbi__result_info *ri) {
  ...
  if (strcmp(headerToken, "#?RADIANCE") != 0 && strcmp(headerToken, "#?RGBE") != 0)
    return (float *)(size_t)stbi__err("not HDR : Corrupt HDR image");
  ...
  if (!valid)
    return (float *)(size_t)stbi__err("unsupported format : Unsupported HDR format");

  // Parse width and height
  // can't use sscanf() if we're not using stdio!
  token = stbi__hdr_gettoken(s, buffer);
  if (strncmp(token, "-Y ", 3))
    return (float *)(size_t)stbi__err("unsupported data layout : Unsupported HDR format");
  token += 3;
  ...
  if (height > MAX_DIMENSIONS)
    return (float *)(size_t)stbi__err("Very large image (corrupt?)");
  ...
  *y = height;
  ...
  if (!stbi__mad4sizes_valid(width, height, req_comp, sizeof(float), 0))
    return (float *)(size_t)stbi__err("HDR image is too large");
...
static void *stbi__pnm_load(stbi__context *s, int *x, int *y, int *comp, int req_comp, stbi__result_info *ri) {
  ...
  *y = s->img_y;
  ...
  if (!stbi__mad4sizes_valid(s->img_n, s->img_x, s->img_y, ri->bits_per_channel / 8, 0))
    return (ubyte *)(size_t)stbi__err("PNM too large");

  out = (ubyte *)stbi__malloc_mad4(s->img_n, s->img_x, s->img_y, ri->bits_per_channel / 8, 0);
  if (!out)
    return (ubyte *)(size_t)stbi__err("outofmem : Out of memory");
  ...
  if (req_comp && req_comp != s->img_n) {
    if (ri->bits_per_channel == 16) {
      out = (ubyte *)stbi__convert_format16((ushrt *)out, s->img_n, req_comp, s->img_x, s->img_y);
    } else {

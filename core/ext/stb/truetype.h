...
extern void stbtt_GetBakedQuad(const stbtt_bakedchar *chardata, ivec2 p, // same data as above
                               int char_index,                                  // character to display
                               vec2 *pos,                        // pointers to current position in screen pixel space
...
extern void stbtt_GetPackedQuad(const stbtt_packedchar *chardata, ivec2 p, // same data as above
                                int char_index,                                   // character to display
                                vec2 *pos,                         // pointers to current position in screen pixel space
                                stbtt_aligned_quad *q,                            // output: quad to draw
...
extern unsigned char *stbtt_GetCodepointBitmap(const stbtt_fontinfo *info, vec2 scale, int codepoint, ivec2 *out_size, ivec2 *off);
...
extern unsigned char *stbtt_GetCodepointBitmapSubpixel(const stbtt_fontinfo *info, vec2 scale, vec2 shift, int codepoint, ivec2 *out_size, ivec2 *off);
// the same as stbtt_GetCodepoitnBitmap, but you can specify a subpixel
// shift for the character

extern void stbtt_MakeCodepointBitmap(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, int codepoint);
// the same as stbtt_GetCodepointBitmap, but you pass in storage for the bitmap
// in the form of 'output', with row spacing of 'out_stride' bytes. the bitmap
// is clipped to out_w/out_h bytes. Call stbtt_GetCodepointBitmapBox to get the
// width and height and positioning info for it first.

extern void stbtt_MakeCodepointBitmapSubpixel(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, vec2 shift, int codepoint);
// same as stbtt_MakeCodepointBitmap, but you can specify a subpixel
// shift for the character

extern void stbtt_MakeCodepointBitmapSubpixelPrefilter(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, vec2 shift, int oversample_x, int oversample_y, vec2 *sub, int codepoint);
...
extern unsigned char *stbtt_GetGlyphBitmap(const stbtt_fontinfo *info, vec2 scale, int glyph, ivec2 *out_size, ivec2 *off);
extern unsigned char *stbtt_GetGlyphBitmapSubpixel(const stbtt_fontinfo *info, vec2 scale, vec2 shift, int glyph, ivec2 *out_size, ivec2 *off);
extern void stbtt_MakeGlyphBitmap(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, int glyph);
extern void stbtt_MakeGlyphBitmapSubpixel(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, vec2 shift, int glyph);
extern void stbtt_MakeGlyphBitmapSubpixelPrefilter(const stbtt_fontinfo *info, stbtt__bitmap gbm, vec2 scale, vec2 shift, int oversample_x, int oversample_y, vec2 *sub, int glyph);
...
extern void stbtt_Rasterize(stbtt__bitmap *result,        // 1-channel bitmap to draw into
                            ...
                            vec2 scl, // scale applied to input vertices
                            vec2 shift, // translation applied to input vertices
                            ivec2 off,         // another translation applied to input
...
extern unsigned char *stbtt_GetGlyphSDF(const stbtt_fontinfo *info, float scale, int glyph, int padding, unsigned char onedge_value, float pixel_dist_scale, ivec2 *out_size, ivec2 *off);
extern unsigned char *stbtt_GetCodepointSDF(const stbtt_fontinfo *info, float scale, int codepoint, int padding, unsigned char onedge_value, float pixel_dist_scale, ivec2 *out_size, ivec2 *off);
